...
- Prefer section-wide edit buttons over per-field inline edit toggles on view pages — each section gets one edit button to toggle all its fields between read-only and editable state, rather than individual pencil icons per field. Confidence: 0.85
...
- Hide zero-count passenger type rows completely from cost/margin tables (Adult, GBE/Youth, Child, Infant with 0 count) rather than greying them out with reduced opacity. Confidence: 0.75
