        ...
        <table role="presentation" width="680" cellpadding="0" cellspacing="0"
            ...
            <td style="background:#FEF2F2;border-bottom:1px solid #FECACA;padding:12px 24px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td>
                    <div
                      style="font-size:11px;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;color:#B91C1C;">
                      Total Refund Requested</div>
                    <div style="font-size:24px;font-weight:800;color:#DC2626;line-height:1.3;">
                      &pound;{{ number_format($refund->refund_amount, 2) }}</div>
                  </td>
                  <td align="right" style="vertical-align:middle;">
                    <span
                      style="display:inline-block;font-size:11px;font-weight:700;letter-spacing:0.05em;text-transform:uppercase;color:#ffffff;background:#DC2626;padding:4px 12px;border-radius:16px;">Booking
          ...
          <tr>
            <td style="padding:16px 24px 2px 24px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0"
                ...
                <tr>
                  <td
                    style="padding:6px 0;border-bottom:1px solid #F1F5F9;font-size:12px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:0.04em;width:160px;vertical-align:top;">
                    Customer</td>
                  <td style="padding:6px 0;border-bottom:1px solid #F1F5F9;font-size:14px;color:#1E293B;">
                    {{ $booking->booker_name ?: 'N/A' }}</td>
                </tr>
                <tr>
                  <td
                    style="padding:6px 0;border-bottom:1px solid #F1F5F9;font-size:12px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:0.04em;vertical-align:top;">
                    Request Sent By</td>
                  <td style="padding:6px 0;border-bottom:1px solid #F1F5F9;font-size:14px;color:#1E293B;">
                    {{ $refund->requestedBy->name ?? 'N/A' }}</td>
                </tr>
                <tr>
                  <td
                    style="padding:6px 0;font-size:12px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:0.04em;vertical-align:top;">
                    Penalties</td>
          ...
          <tr>
            <td style="padding:6px 24px 20px 24px;">
              <div
                style="font-size:12px;font-weight:700;color:#332E9E;text-transform:uppercase;letter-spacing:0.06em;margin:10px 0 6px 0;">
                Pax Details</div>
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0"
                style="border-collapse:collapse;border-radius:8px;overflow:hidden;border:1px solid #E2E8F0;">
                <tr style="background:#332E9E;">
                  <th
                    style="padding:7px 8px;text-align:left;font-size:11px;font-weight:700;color:#ffffff;text-transform:uppercase;letter-spacing:0.02em;">
                    S.No.</th>
                  <th
                    style="padding:7px 8px;text-align:left;font-size:11px;font-weight:700;color:#ffffff;text-transform:uppercase;letter-spacing:0.02em;">
                    Passenger</th>
                ...
                @foreach ($refund->passengers as $i => $rp)
                  <tr style="background:{{ $i % 2 === 0 ? '#ffffff' : '#F8FAFF' }};">
                    <td style="padding:8px;font-size:12px;color:#64748B;border-top:1px solid #F1F5F9;">
                      {{ $i + 1 }}</td>
                    <td style="padding:8px;font-size:12px;color:#1E293B;font-weight:600;border-top:1px solid #F1F5F9;">
                      {{ $rp->passenger?->display_name ?? 'N/A' }}</td>
                    <td
                      style="padding:8px;font-size:12px;color:#1E293B;font-family:'Courier New',monospace;border-top:1px solid #F1F5F9;">
                      {{ $rp->gds_locator ?: 'N/A' }}</td>
                    <td
                      style="padding:8px;font-size:12px;color:#1E293B;font-family:'Courier New',monospace;border-top:1px solid #F1F5F9;">
                      {{ $rp->airline_locator ?: 'N/A' }}</td>
                    <td
                      style="padding:8px;font-size:12px;color:#1E293B;font-family:'Courier New',monospace;border-top:1px solid #F1F5F9;">
                      {{ $rp->airline_waiver_code ?: 'N/A' }}</td>
                    <td
                      style="padding:8px;font-size:12px;color:#1E293B;font-family:'Courier New',monospace;border-top:1px solid #F1F5F9;">
                      {{ $rp->e_ticket_number ?: 'N/A' }}</td>
                  </tr>
                @endforeach
              </table>
            </td>
