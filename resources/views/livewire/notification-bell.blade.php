  ...
  <div x-show="open" x-cloak
    ...
    <div class="d-flex align-items-center justify-content-between px-4 py-3" style="border-bottom:1px solid rgba(51,46,158,.07);">
      <div class="d-flex align-items-center gap-2">
        <span class="fw-bold" style="font-size:.85rem;color:#0F172A;">Notifications</span>
        @if($notifications->count() > 0)
          <span style="background:rgba(51,46,158,.10);color:#332E9E;border-radius:20px;font-size:.62rem;font-weight:700;padding:1px 7px;">{{ $notifications->count() }}</span>
        @endif
      </div>
      <button type="button" @click="open=false;$wire.close()" style="background:none;border:none;color:#94A3B8;cursor:pointer;font-size:.9rem;padding:2px;line-height:1;">✕</button>
    </div>
    ...
    @if($notifications->isEmpty())
      <div class="text-center py-5" style="color:#C4C9D4;">
        ...
        <div style="font-size:.75rem;color:#94A3B8;">All clear - no alerts right now</div>
      </div>
    @else
        ...
        @foreach($notifications as $notif)
          <a href="{{ $notif['url'] }}" wire:click="close"
            ...
            <div style="width:34px;height:34px;border-radius:10px;background:{{ $notif['bg'] }};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
              <i class="ph {{ $notif['icon'] }}" style="font-size:.95rem;color:{{ $notif['color'] }};"></i>
            </div>
            <div class="flex-grow-1">
              <div style="font-size:.78rem;font-weight:600;color:#1E293B;line-height:1.3;">{{ $notif['title'] }}</div>
              <div style="font-size:.68rem;color:#94A3B8;margin-top:2px;">{{ $notif['sub'] }}</div>
            </div>
            <i class="ph ph-arrow-right" style="color:#CBD5E1;font-size:.78rem;margin-top:4px;flex-shrink:0;"></i>
          </a>
        @endforeach
      </div>
    @endif
    ...
    <div class="px-4 py-2" style="border-top:1px solid rgba(51,46,158,.06);background:#F8FAFF;text-align:center;">
      <span style="font-size:.67rem;color:#CBD5E1;">Live data · based on your role</span>
    </div>
  </div>
</div>
