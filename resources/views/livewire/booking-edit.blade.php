...
.be-hdr h2 { font-size:1.056rem;font-weight:700;color:#0F172A;margin:0; }
.be-body { padding:20px 22px; }
.be-label { display:block;font-size:0.816rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#475569;margin-bottom:5px; }
.be-input { border-radius:10px;border:1.5px solid rgba(51,46,158,.12);padding:.45rem .75rem;font-size:0.996rem;width:100%;background:#fff;color:#1E293B;transition:border-color .15s; }
.be-input:focus { border-color:#332E9E;box-shadow:0 0 0 3px rgba(51,46,158,.08);outline:none; }
.be-select { border-radius:10px;border:1.5px solid rgba(51,46,158,.12);padding:.42rem .75rem;font-size:0.996rem;width:100%;background:#fff;color:#1E293B; }
.be-select:focus { border-color:#332E9E;outline:none; }
.pax-table th { font-size:0.72rem;text-transform:uppercase;letter-spacing:.05em;color:#475569;font-weight:700;padding:8px 10px;border:none;background:rgba(51,46,158,.03);white-space:nowrap; }
.pax-table td { padding:7px 8px;border-color:rgba(51,46,158,.05);vertical-align:middle; }
.pax-table input,.pax-table select { border-radius:7px;border:1px solid rgba(51,46,158,.12);padding:4px 7px;font-size:0.9rem;background:#fff;color:#1E293B;width:100%; }
...
.be-counter-lbl { font-size:0.78rem;font-weight:700;padding:0 10px;border-right:1px solid rgba(51,46,158,.08); }
.be-counter-btn { width:28px;height:32px;border:none;background:transparent;cursor:pointer;font-size:1.2rem;color:#332E9E;transition:background .12s; }
.be-counter-btn:hover { background:rgba(51,46,158,.08); }
.be-counter-val { width:28px;text-align:center;font-size:1.08rem;font-weight:700;color:#1E293B; }
</style>
...
<div class="d-flex align-items-start justify-content-between mb-4 flex-wrap gap-3">
  <div>
    <div style="font-size:0.864rem;color:#475569;margin-bottom:4px;">
      ...
      <a href="{{ route('bookings.index') }}" style="color:#475569;text-decoration:none;">Bookings</a> ›
      <a href="{{ route('bookings.show', $booking) }}" style="color:#475569;text-decoration:none;">#{{ $booking->booking_number }}</a> ›
    ...
    <div style="font-size:0.888rem;color:#475569;margin-top:3px;">Last updated {{ $booking->updated_at->diffForHumans() }}</div>
  </div>
  <div class="d-flex gap-2">
    <a href="{{ route('bookings.show', $booking) }}" style="padding:8px 18px;border-radius:10px;border:1.5px solid rgba(51,46,158,.15);color:#374151;font-size:0.972rem;font-weight:600;text-decoration:none;display:inline-flex;align-items:center;gap:5px;">
      <i class="ph ph-arrow-left"></i> Cancel
    </a>
    <button type="submit" form="edit-booking-form"
      style="background:linear-gradient(135deg,#FF6B35,#FF8C5A);color:#fff;border:none;border-radius:10px;padding:8px 22px;font-size:0.972rem;font-weight:700;cursor:pointer;box-shadow:0 3px 12px rgba(255,107,53,.3);display:inline-flex;align-items:center;gap:5px;">
...
@if(session()->has('success'))
  <div class="d-flex align-items-center gap-2 mb-3 px-4 py-3" style="background:rgba(22,163,74,.08);border-radius:12px;border:1px solid rgba(22,163,74,.2);color:#15803D;font-size:0.984rem;">
    <i class="ph ph-check-circle" style="font-size:1.32rem;"></i> {{ session('success') }}
  </div>
@endif
@if($errors->any())
  <div class="mb-3 px-4 py-3" style="background:rgba(220,38,38,.06);border-radius:12px;border:1px solid rgba(220,38,38,.15);font-size:0.948rem;color:#DC2626;">
...
<div class="be-section">
  <div class="be-hdr">
    <div class="be-icon" style="background:rgba(51,46,158,.08);"><i class="ph ph-clipboard-text" style="color:#332E9E;font-size:1.2rem;"></i></div>
...
<div class="be-section">
  <div class="be-hdr">
    <div class="be-icon" style="background:rgba(14,165,233,.08);"><i class="ph ph-user-circle" style="color:#0EA5E9;font-size:1.2rem;"></i></div>
    <h2>Caller / Booker</h2>
  </div>
  <div class="be-body">
    <div class="row g-3">
...
<div class="be-section">
  <div class="be-hdr">
    <div class="be-icon" style="background:rgba(22,163,74,.08);"><i class="ph ph-users" style="color:#16A34A;font-size:1.2rem;"></i></div>
    <h2>Travellers</h2>
    <span class="ms-auto" style="font-size:0.864rem;color:#475569;">{{ $this->totalPassengers }} passenger{{ $this->totalPassengers !== 1 ? 's' : '' }}</span>
  </div>
  <div class="be-body" style="padding-bottom:12px;">
        ...
        <tbody>
          @foreach($passengers as $i => $p)
            ...
            <tr style="{{ $loop->even ? 'background:#FAFBFF;' : '' }}">
              <td style="color:#475569;font-size:0.864rem;font-weight:600;">{{ $i+1 }}</td>
              <td><span style="background:{{ $col }}15;color:{{ $col }};padding:2px 8px;border-radius:20px;font-size:0.744rem;font-weight:700;">{{ $ptLabels[$p['type']] ?? '-' }}</span></td>
              <td><span style="font-size:0.84rem;font-weight:700;color:{{ $col }};">{{ $ptc ?: '-' }}</span></td>
              ...
              <td><x-date-picker :modelName="'passengers.'.$i.'.date_of_birth'" :compact="true" />@error("passengers.{$i}.date_of_birth")<small style="color:#DC2626;font-size:0.744rem;">{{ $message }}</small>@enderror</td>
    ...
    @else
      <div class="text-center py-3" style="color:#475569;font-size:0.912rem;">Add travellers using the counters above.</div>
    @endif
  </div>
</div>
...
<div class="be-section">
  <div class="be-hdr" style="{{ !$canEditFlightHotel ? 'background:rgba(51,46,158,.03);' : '' }}">
    <div class="be-icon" style="background:rgba(14,165,233,.08);"><i class="ph ph-airplane" style="color:#0EA5E9;font-size:1.2rem;"></i></div>
    <h2>Flight Information</h2>
    @if(!$canEditFlightHotel)
      <span class="ms-auto d-flex align-items-center gap-1" style="font-size:0.84rem;color:#475569;font-weight:600;">
        <i class="ph ph-lock" style="font-size:0.96rem;"></i> Manager access only
      </span>
    @endif
  </div>
  @if(!$canEditFlightHotel)
    ...
    <div class="be-body" style="background:rgba(51,46,158,.02);">
      @if($booking->flightDetail)
        ...
        <div class="d-flex align-items-center gap-2 mb-3 px-3 py-2" style="background:rgba(14,165,233,.06);border-radius:10px;border:1px solid rgba(14,165,233,.15);">
          <i class="ph ph-info" style="color:#0EA5E9;font-size:1.2rem;"></i>
          <span style="font-size:0.912rem;color:#0369A1;">Flight details can only be modified by a Manager. Contact your manager to add or change flight information.</span>
        </div>
        <div class="row g-3">
          @if($fd->pnr)<div class="col-md-3"><div class="be-label">PNR</div><div style="font-size:0.996rem;font-weight:600;font-family:monospace;color:#1E293B;">{{ $fd->pnr }}</div></div>@endif
          @if($fd->airline)<div class="col-md-2"><div class="be-label">Airline</div><div style="font-size:1.08rem;font-weight:800;color:#0EA5E9;">{{ $fd->airline }}</div></div>@endif
          @if($fd->departure_airport || $fd->arrival_airport)
            <div class="col-md-3"><div class="be-label">Route</div><div style="font-size:1.056rem;font-weight:700;color:#1E293B;">{{ strtoupper($fd->departure_airport) }} → {{ strtoupper($fd->arrival_airport) }}</div></div>
          @endif
          @if($fd->departure_date)<div class="col-md-2"><div class="be-label">Departure</div><div style="font-size:0.996rem;color:#374151;">{{ \Carbon\Carbon::parse($fd->departure_date)->format('d M Y') }}</div></div>@endif
          @if($fd->flight_type)<div class="col-md-2"><div class="be-label">Flight Type</div><div style="font-size:0.996rem;font-weight:700;color:#1E293B;">{{ $fd->flight_type === 'one_way' ? 'One Way' : 'Return' }}</div></div>@endif
          @if(($fd->flight_type ?? 'return') !== 'one_way' && $fd->return_date)<div class="col-md-2"><div class="be-label">Return</div><div style="font-size:0.996rem;color:#374151;">{{ \Carbon\Carbon::parse($fd->return_date)->format('d M Y') }}</div></div>@endif
        </div>
      @else
        <div class="d-flex align-items-center gap-2 px-3 py-2" style="background:rgba(51,46,158,.04);border-radius:10px;">
          <i class="ph ph-lock" style="color:#475569;"></i>
          <span style="font-size:0.912rem;color:#475569;">No flight details added yet. Ask your manager to add flight information.</span>
        </div>
      @endif
    </div>
  @else
    ...
    <div class="row g-3 mb-3">
      <div class="col-md-6"><label class="be-label">PNR Details</label><textarea wire:model="flight_pnr" class="be-input" rows="3" placeholder="Paste PNR content..." style="font-family:monospace;font-size:0.912rem;"></textarea></div>
    ...
    <div class="row g-3 align-items-end">
      ...
      <div class="col-md-3 d-flex gap-4 pb-1">
        <label class="d-flex align-items-center gap-2 mb-0 cursor-pointer" style="cursor:pointer;font-size:0.96rem;font-weight:600;">
...
<div class="be-section">
  <div class="be-hdr" style="{{ !$canEditFlightHotel ? 'background:rgba(51,46,158,.03);' : '' }}">
    <div class="be-icon" style="background:rgba(124,58,237,.08);"><i class="ph ph-buildings" style="color:#7C3AED;font-size:1.2rem;"></i></div>
    ...
    @else
      <span style="font-size:0.84rem;color:#475569;margin-left:4px;">(optional - add if customer wants hotel)</span>
    @endif
  </div>
  @if(!$canEditFlightHotel)
    <div class="be-body" style="background:rgba(51,46,158,.02);">
      @if($booking->hotels?->isNotEmpty())
        <div class="d-flex align-items-center gap-2 mb-3 px-3 py-2" style="background:rgba(124,58,237,.06);border-radius:10px;border:1px solid rgba(124,58,237,.15);">
          <i class="ph ph-info" style="color:#7C3AED;font-size:1.2rem;"></i>
          <span style="font-size:0.912rem;color:#6D28D9;">Hotel details can only be modified by a Manager.</span>
        </div>
        @foreach($booking->hotels as $hotel)
          <div class="mb-3" style="border:1px solid rgba(124,58,237,.10);border-radius:10px;padding:14px 16px;background:#fff;">
            <div class="fw-bold mb-2" style="font-size:0.984rem;color:#7C3AED;">{{ $hotel->hotel_name }}</div>
            <div class="row g-2" style="font-size:0.936rem;">
              <div class="col-md-3"><span style="color:#475569;">City:</span> {{ $hotel->city ?? '-' }}</div>
              <div class="col-md-3"><span style="color:#475569;">Status:</span> {{ ucfirst($hotel->booking_status) }}</div>
              <div class="col-md-3"><span style="color:#475569;">Check In:</span> {{ $hotel->check_in?->format('d M Y') ?? '-' }}</div>
              <div class="col-md-3"><span style="color:#475569;">Check Out:</span> {{ $hotel->check_out?->format('d M Y') ?? '-' }}</div>
              <div class="col-md-3"><span style="color:#475569;">Cost:</span> £{{ number_format($hotel->actual_cost, 2) }}</div>
              <div class="col-md-3"><span style="color:#475569;">Selling:</span> £{{ number_format($hotel->selling_price, 2) }}</div>
            </div>
            @if($hotel->rooms->isNotEmpty())
              <div class="mt-2 pt-2" style="border-top:1px solid rgba(124,58,237,.08);">
                <span style="font-size:0.84rem;color:#7C3AED;font-weight:600;">Rooms ({{ $hotel->rooms->count() }})</span>
                <div class="d-flex flex-wrap gap-2 mt-1">
                  @foreach($hotel->rooms as $room)
                    <span class="badge" style="background:rgba(124,58,237,.06);color:#5A6080;font-size:0.804rem;padding:3px 8px;border-radius:6px;">
      ...
      @else
        <div class="d-flex align-items-center gap-2 px-3 py-2" style="background:rgba(51,46,158,.04);border-radius:10px;">
          <i class="ph ph-lock" style="color:#475569;"></i>
          <span style="font-size:0.912rem;color:#475569;">No hotel added. Ask your manager to add hotel details to this booking.</span>
        </div>
      @endif
    </div>
  @else
    <div class="be-body">
      @foreach($hotels as $hi => $hotel)
        <div class="mb-3" style="border:1px solid rgba(124,58,237,.08);border-radius:10px;overflow:hidden;">
          <div class="d-flex align-items-center justify-content-between px-3 py-2" style="background:rgba(124,58,237,.04);">
            <span class="fw-semibold" style="font-size:0.912rem;color:#7C3AED;">Hotel {{ $hi + 1 }}</span>
            <button type="button" wire:click="removeHotel({{ $hi }})" style="background:rgba(220,38,38,.08);border:none;color:#DC2626;border-radius:6px;padding:2px 10px;font-size:0.816rem;cursor:pointer;">Remove</button>
          </div>
          <div class="p-3">
            ...
            @foreach($hotel['rooms'] ?? [] as $ri => $room)
              <div class="mt-2 p-2" style="background:rgba(124,58,237,.03);border-radius:8px;border:1px solid rgba(124,58,237,.06);">
                <span style="font-size:0.816rem;font-weight:600;color:#5A6080;">Room {{ $ri + 1 }}</span>
                <div class="row g-2 mt-1">
                  <div class="col-md-4"><label class="be-label" style="font-size:0.756rem;">Room Type</label><input type="text" wire:model="hotels.{{ $hi }}.rooms.{{ $ri }}.room_type" class="be-input" style="font-size:0.876rem;padding:.35rem .5rem;" placeholder="Double"></div>
                  <div class="col-md-3"><label class="be-label" style="font-size:0.756rem;">Occupants</label><input type="number" wire:model="hotels.{{ $hi }}.rooms.{{ $ri }}.occupants" class="be-input" style="font-size:0.876rem;padding:.35rem .5rem;" min="1"></div>
                  <div class="col-md-5"><label class="be-label" style="font-size:0.756rem;">Meal Basis</label><x-styled-select-sm :modelName="'hotels.'.$hi.'.rooms.'.$ri.'.meal_basis'" :options="[['value'=>'room_only','label'=>'Room Only'],['value'=>'breakfast','label'=>'Breakfast'],['value'=>'half_board','label'=>'Half Board'],['value'=>'full_board','label'=>'Full Board'],['value'=>'all_inclusive','label'=>'All Inclusive']]" placeholder="Select" /></div>
                </div>
              </div>
            @endforeach
          </div>
      ...
      <button type="button" wire:click="addHotel"
        style="background:rgba(124,58,237,.07);border:1.5px dashed rgba(124,58,237,.2);color:#7C3AED;border-radius:10px;padding:6px 16px;font-size:0.912rem;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:5px;margin-top:4px;">
...
<div class="be-section">
  <div class="be-hdr" style="{{ !$canEditFlightHotel ? 'background:rgba(51,46,158,.03);' : '' }}">
    <div class="be-icon" style="background:rgba(51,46,158,.08);"><i class="ph ph-van" style="color:#332E9E;font-size:1.2rem;"></i></div>
    <h2>Transfers</h2>
    @if(!$canEditFlightHotel)
      <span class="ms-auto d-flex align-items-center gap-1" style="font-size:0.84rem;color:#475569;font-weight:600;">
        <i class="ph ph-lock" style="font-size:0.96rem;"></i> Manager access only
    ...
    <div class="be-body" style="background:rgba(51,46,158,.02);">
      @if($booking->transfers?->isNotEmpty())
        ...
        @if($pickups->isNotEmpty())
          <div class="mb-2"><span style="font-size:0.864rem;font-weight:600;color:#332E9E;">Pickups ({{ $pickups->count() }})</span></div>
          @foreach($pickups as $t)
            <div class="mb-2 px-3 py-2" style="background:rgba(51,46,158,.03);border-radius:8px;font-size:0.9rem;">
              ...
              @if($t->route)<div class="text-muted mt-1" style="font-size:0.84rem;">Route: {{ $t->route }}</div>@endif
            </div>
          @endforeach
        @endif
        @if($dropoffs->isNotEmpty())
          <div class="mb-2 mt-3"><span style="font-size:0.864rem;font-weight:600;color:#D83F87;">Dropoffs ({{ $dropoffs->count() }})</span></div>
          @foreach($dropoffs as $t)
            <div class="mb-2 px-3 py-2" style="background:rgba(216,63,135,.03);border-radius:8px;font-size:0.9rem;">
              <span class="fw-semibold">{{ $t->location }}</span>
              @if($t->date_time)<span class="text-muted ms-2">{{ $t->date_time->format('d M Y, H:i') }}</span>@endif
              @if($t->route)<div class="text-muted mt-1" style="font-size:0.84rem;">Route: {{ $t->route }}</div>@endif
            </div>
        ...
        @endif
      @else
        <span style="font-size:0.912rem;color:#475569;">No transfers added.</span>
      @endif
    </div>
  @else
    <div class="be-body">
      <div class="mb-3">
        <div class="d-flex justify-content-between align-items-center mb-2">
          <span class="fw-semibold" style="font-size:0.912rem;color:#332E9E;">Pickups ({{ count($transferPickups) }})</span>
          <button type="button" wire:click="addPickup" style="background:rgba(51,46,158,.07);border:1px solid rgba(51,46,158,.15);color:#332E9E;border-radius:7px;padding:3px 12px;font-size:0.84rem;cursor:pointer;">+ Add Pickup</button>
        </div>
        @foreach($transferPickups as $pi => $pickup)
          <div class="mb-2 p-2" style="border:1px solid rgba(51,46,158,.08);border-radius:8px;">
            <div class="d-flex justify-content-between mb-1"><span style="font-size:0.816rem;font-weight:600;color:#5A6080;">Pickup {{ $pi + 1 }}</span><button type="button" wire:click="removePickup({{ $pi }})" style="background:none;border:none;color:#DC2626;font-size:0.78rem;cursor:pointer;">Remove</button></div>
            <div class="row g-1">
              <div class="col-md-5"><input type="text" wire:model="transferPickups.{{ $pi }}.location" class="be-input" style="font-size:0.864rem;padding:.3rem .45rem;" placeholder="Location"></div>
              <div class="col-md-3"><x-date-picker :modelName="'transferPickups.'.$pi.'.date_time'" :compact="true" :isDateTime="true" /></div>
              <div class="col-md-2"><input type="text" wire:model="transferPickups.{{ $pi }}.flight_number" class="be-input" style="font-size:0.864rem;padding:.3rem .45rem;" placeholder="Flight #"></div>
              <div class="col-md-2"><input type="text" wire:model="transferPickups.{{ $pi }}.route" class="be-input" style="font-size:0.864rem;padding:.3rem .45rem;" placeholder="Route"></div>
            </div>
          </div>
        @endforeach
      </div>
      <div class="mb-3">
        <div class="d-flex justify-content-between align-items-center mb-2">
          <span class="fw-semibold" style="font-size:0.912rem;color:#D83F87;">Dropoffs ({{ count($transferDropoffs) }})</span>
          <button type="button" wire:click="addDropoff" style="background:rgba(216,63,135,.07);border:1px solid rgba(216,63,135,.15);color:#D83F87;border-radius:7px;padding:3px 12px;font-size:0.84rem;cursor:pointer;">+ Add Dropoff</button>
        </div>
        @foreach($transferDropoffs as $di => $dropoff)
          <div class="mb-2 p-2" style="border:1px solid rgba(216,63,135,.08);border-radius:8px;">
            <div class="d-flex justify-content-between mb-1"><span style="font-size:0.816rem;font-weight:600;color:#5A6080;">Dropoff {{ $di + 1 }}</span><button type="button" wire:click="removeDropoff({{ $di }})" style="background:none;border:none;color:#DC2626;font-size:0.78rem;cursor:pointer;">Remove</button></div>
            <div class="row g-1 mb-1">
              <div class="col-md-5"><input type="text" wire:model="transferDropoffs.{{ $di }}.location" class="be-input" style="font-size:0.864rem;padding:.3rem .45rem;" placeholder="Location"></div>
              <div class="col-md-3"><x-date-picker :modelName="'transferDropoffs.'.$di.'.date_time'" :compact="true" :isDateTime="true" /></div>
              <div class="col-md-2"><input type="text" wire:model="transferDropoffs.{{ $di }}.flight_number" class="be-input" style="font-size:0.864rem;padding:.3rem .45rem;" placeholder="Flight #"></div>
            </div>
            <textarea wire:model="transferDropoffs.{{ $di }}.route" class="be-input" rows="2" style="font-size:0.864rem;padding:.3rem .45rem;" placeholder="Route description..."></textarea>
          </div>
        @endforeach
      </div>
    </div>
...
<div class="be-section">
  <div class="be-hdr">
    <div class="be-icon" style="background:rgba(124,58,237,.08);"><i class="ph ph-file-text" style="color:#7C3AED;font-size:1.2rem;"></i></div>
    <h2>Documents</h2>
  </div>
  <div class="be-body">
    @foreach($existingDocuments as $ed)
      <div class="d-flex align-items-center justify-content-between mb-2 px-3 py-2" style="background:#F8FAFF;border-radius:9px;">
        <span style="font-size:0.936rem;"><i class="ph ph-file me-1" style="color:#7C3AED;"></i>{{ $ed['file_name'] ?? 'File' }}</span>
        <button type="button" wire:click="deleteExistingDocument({{ $ed['id'] ?? 0 }})"
          style="background:rgba(220,38,38,.08);border:none;color:#DC2626;border-radius:7px;padding:3px 10px;font-size:0.84rem;cursor:pointer;">Remove</button>
      </div>
    @endforeach
    @foreach($newDocuments as $i => $doc)
      <div class="d-flex gap-2 align-items-center mb-2">
        <input type="file" wire:model="newDocuments.{{ $i }}" class="be-input" style="font-size:0.936rem;padding:.35rem .65rem;">
        ...
        <button type="button" wire:click="removeDocument({{ $i }})" style="background:rgba(220,38,38,.08);border:none;color:#DC2626;border-radius:7px;padding:5px 10px;font-size:0.96rem;cursor:pointer;flex-shrink:0;">✕</button>
      </div>
    @endforeach
    <button type="button" wire:click="addDocument"
      style="background:rgba(51,46,158,.07);border:1.5px dashed rgba(51,46,158,.2);color:#332E9E;border-radius:10px;padding:6px 16px;font-size:0.912rem;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:5px;margin-top:4px;">
...
<div class="col-lg-4">
  ...
  <div class="be-section">
    <div class="be-hdr">
      <div class="be-icon" style="background:rgba(14,165,233,.08);"><i class="ph ph-credit-card" style="color:#0EA5E9;font-size:1.2rem;"></i></div>
      <h2>Payment</h2>
    </div>
    <div class="be-body">
  ...
  <div class="be-section">
    <div class="be-hdr">
      <div class="be-icon" style="background:rgba(51,46,158,.08);"><i class="ph ph-currency-pound" style="color:#332E9E;font-size:1.2rem;"></i></div>
      <h2>Cost &amp; Margins</h2>
    </div>
    <div class="be-body">
      @php
      ...
      @endphp
      <div class="d-flex justify-content-between mb-2" style="font-size:0.96rem;">
        <span style="color:#475569;">Total Cost</span>
        <span class="fw-semibold">£{{ number_format($totalCost, 2) }}</span>
      </div>
      <div class="d-flex justify-content-between mb-2" style="font-size:0.96rem;">
        <span style="color:#475569;">Total Sold</span>
      ...
      <div class="d-flex justify-content-between align-items-center px-3 py-2 rounded-3" style="background:{{ $margin >= 0 ? 'rgba(22,163,74,.08)' : 'rgba(220,38,38,.08)' }};">
        <span style="font-size:0.936rem;font-weight:700;color:{{ $margin >= 0 ? '#15803D' : '#DC2626' }};">Total Margin</span>
        <span style="font-size:1.26rem;font-weight:800;color:{{ $margin >= 0 ? '#15803D' : '#DC2626' }};">£{{ number_format($margin, 2) }}</span>
      </div>
    </div>
  </div>
  ...
  <div class="be-section">
    <div class="be-hdr" style="background:linear-gradient(135deg,#332E9E,#4A45B5);border-radius:16px 16px 0 0;">
      <div class="be-icon" style="background:rgba(255,255,255,.15);"><i class="ph ph-clock-countdown" style="color:#fff;font-size:1.2rem;"></i></div>
      <h2 style="color:#fff;">Activity Log</h2>
      <span style="font-size:0.72rem;color:rgba(255,255,255,.45);background:rgba(255,255,255,.10);padding:1px 8px;border-radius:20px;margin-left:4px;">auto-recorded</span>
      <span class="ms-auto" style="font-size:0.78rem;color:rgba(255,255,255,.45);">{{ count($activity_log_entries) }} event{{ count($activity_log_entries)!==1?'s':'' }}</span>
    </div>

    {{-- Timeline --}}
    <div style="max-height:280px;overflow-y:auto;padding:16px 20px 4px;">
      @if(empty($activity_log_entries))
        <div class="text-center py-3" style="color:#475569;font-size:0.876rem;">No activity yet.</div>
      @else
            ...
            <div class="flex-grow-1" style="{{ !$isLast ? 'padding-bottom:16px;' : 'padding-bottom:8px;' }}">
              <div class="d-flex align-items-start justify-content-between gap-2 flex-wrap">
                <div class="d-flex align-items-center gap-2 flex-wrap">
                  <span class="fw-semibold" style="font-size:0.912rem;color:#1E293B;">{{ $entry['agent'] }}</span>
                  <span style="font-size:0.852rem;color:#475569;">{{ $entry['action'] }}</span>
                  <span style="font-size:0.684rem;font-weight:700;color:{{ $cfg['pc'] }};background:{{ $cfg['pbg'] }};padding:1px 6px;border-radius:20px;text-transform:uppercase;letter-spacing:.04em;">{{ $cfg['lbl'] }}</span>
                </div>
                <span style="font-size:0.744rem;color:#475569;white-space:nowrap;padding-top:2px;">{{ $entry['timestamp'] }}</span>
              </div>
              @if(!empty($entry['detail']))
                <div class="mt-1 px-2 py-1" style="background:{{ ($entry['type']??'')!=='note' ? $cfg['pbg'] : 'rgba(51,46,158,.04)' }};border-radius:6px;border-left:3px solid {{ $cfg['dot'] }};">
                  <span style="font-size:0.828rem;color:#475569;font-style:italic;">{{ $entry['detail'] }}</span>
                </div>
              @elseif($canReason)
                @if($reasonEditIndex === $i)
                  <div class="mt-1 d-flex gap-1 align-items-center">
                    <input type="text" wire:model="reasonEditText" placeholder="Why did you do this?"
                      ...
                      style="font-size:0.864rem;border:1.5px solid rgba(51,46,158,.2);border-radius:7px;padding:4px 10px;flex:1;outline:none;color:#374151;background:#F8FAFF;">
                    <button type="button" wire:click="saveReasonEdit" style="font-size:0.816rem;background:#332E9E;color:#fff;border:none;border-radius:7px;padding:4px 12px;cursor:pointer;font-weight:600;">Save</button>
                    <button type="button" wire:click="cancelReasonEdit" style="font-size:0.84rem;background:transparent;color:#475569;border:1px solid #E2E8F0;border-radius:7px;padding:4px 8px;cursor:pointer;">✕</button>
                  </div>
                @else
                  <div class="mt-1">
                    <button type="button" wire:click="openReasonEdit({{ $i }})"
                      style="font-size:0.804rem;color:#475569;background:none;border:none;padding:0;cursor:pointer;text-decoration:underline;text-underline-offset:2px;">
                      + add reason
                    </button>
                  </div>
                @endif
    ...
    <div class="d-flex gap-2 align-items-center px-4 py-3" style="border-top:1px solid rgba(51,46,158,.07);background:#F8FAFF;">
      <input type="text" wire:model="mandatory_comment" placeholder="Add a note about this edit..."
        ...
        style="border-radius:20px;font-size:0.924rem;" wire:keydown.enter="addActivityEntry">
      <button type="button" wire:click="addActivityEntry"
        class="flex-shrink-0"
        style="background:linear-gradient(135deg,#332E9E,#4A45B5);color:#fff;border:none;border-radius:20px;padding:6px 16px;font-size:0.888rem;font-weight:700;cursor:pointer;white-space:nowrap;">
        Add Note
      </button>
    </div>
    @error('mandatory_comment')<div class="px-4 pb-2" style="font-size:0.864rem;color:#DC2626;">{{ $message }}</div>@enderror
  </div>

  {{-- Save --}}
  <button type="submit"
    style="width:100%;background:linear-gradient(135deg,#FF6B35,#FF8C5A);color:#fff;border:none;border-radius:12px;padding:12px;font-size:1.056rem;font-weight:700;cursor:pointer;box-shadow:0 4px 16px rgba(255,107,53,.3);display:flex;align-items:center;justify-content:center;gap:6px;">
    <i class="ph ph-check-circle" style="font-size:1.32rem;"></i> Save Changes
  </button>
