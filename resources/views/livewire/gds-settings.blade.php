  ...
  <div class="card border-0" style="border-radius:24px;background:linear-gradient(135deg,rgba(255,255,255,0.95) 0%,rgba(255,255,255,0.88) 100%);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);box-shadow:0 8px 40px rgba(51,46,158,.12),0 2px 8px rgba(51,46,158,.06);border:1px solid rgba(255,255,255,.6);">
    <div class="card-body p-4">
      ...
      <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
          <h6 class="fw-bold mb-1" style="color:var(--to-charcoal);font-size:.95rem;">GDS Options</h6>
          <p class="mb-0" style="font-size:.72rem;color:#94A3B8;">Add or remove GDS options that appear in the booking flight form.</p>
        </div>
        <span class="badge" style="background:rgba(51,46,158,.08);color:#332E9E;font-size:.72rem;">{{ count($gdsOptions) }} GDS(s)</span>
      </div>
        ...
        @foreach($gdsOptions as $i => $g)
          <div class="d-flex align-items-center justify-content-between px-3 py-2" style="background:#fff;border:1px solid rgba(51,46,158,.08);border-radius:10px;">
            <div class="d-flex align-items-center gap-3">
              <span class="fw-semibold" style="font-size:.78rem;color:#20242B;">{{ $g['label'] }}</span>
              <span class="badge" style="font-size:.65rem;background:rgba(51,46,158,.06);color:#64748B;font-family:monospace;">{{ $g['value'] }}</span>
            </div>
            <button type="button" wire:click="removeGds({{ $i }})"
              ...
              <span wire:loading wire:target="removeGds({{ $i }})" style="font-size:.65rem;">...</span>
            </button>
          </div>
        @endforeach
      </div>
      ...
      <div class="p-3" style="background:rgba(51,46,158,.03);border-radius:14px;border:1.5px dashed rgba(51,46,158,.15);">
        <h6 class="fw-semibold mb-3" style="font-size:.76rem;color:#332E9E;">Add New GDS</h6>
