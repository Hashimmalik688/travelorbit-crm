  ...
  <div class="card border-0" style="border-radius:24px;background:linear-gradient(135deg,rgba(255,255,255,0.95) 0%,rgba(255,255,255,0.88) 100%);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);box-shadow:0 8px 40px rgba(51,46,158,.12),0 2px 8px rgba(51,46,158,.06);border:1px solid rgba(255,255,255,.6);">
    <div class="card-body p-4">
      ...
      <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
          <h6 class="fw-bold mb-1" style="color:var(--to-charcoal);font-size:1.14rem;">Flight Vendors</h6>
          <p class="mb-0" style="font-size:0.864rem;color:#475569;">Add or remove vendor options that appear in the booking flight form.</p>
        </div>
        <span class="badge" style="background:rgba(51,46,158,.08);color:#332E9E;font-size:0.864rem;">{{ count($vendors) }} vendor(s)</span>
      </div>
        ...
        @foreach($vendors as $i => $v)
          <div class="d-flex align-items-center justify-content-between px-3 py-2" style="background:#fff;border:1px solid rgba(51,46,158,.08);border-radius:10px;">
            <div class="d-flex align-items-center gap-3">
              <span class="fw-semibold" style="font-size:0.936rem;color:#20242B;">{{ $v['label'] }}</span>
              <span class="badge" style="font-size:0.78rem;background:rgba(51,46,158,.06);color:#475569;font-family:monospace;">{{ $v['value'] }}</span>
            </div>
            <button type="button" wire:click="removeVendor({{ $i }})"
              ...
              <span wire:loading wire:target="removeVendor({{ $i }})" style="font-size:0.78rem;">...</span>
            </button>
          </div>
        @endforeach
      </div>
      ...
      <div class="p-3" style="background:rgba(51,46,158,.03);border-radius:14px;border:1.5px dashed rgba(51,46,158,.15);">
        <h6 class="fw-semibold mb-3" style="font-size:0.912rem;color:#332E9E;">Add New Vendor</h6>
